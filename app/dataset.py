...
class DatasetManager:
    ...
    def invert_transform_data(self, y_train_pred=None, y_train=None, y_test_pred=None, y_test=None):
        if y_train_pred!=None:
            y_train_pred = self.scaler.inverse_transform(y_train_pred.detach().cpu().numpy())
        if y_train!=None:
            y_train = self.scaler.inverse_transform(y_train.detach().cpu().numpy())
        if y_test_pred!=None:
            y_test_pred = self.scaler.inverse_transform(y_test_pred.detach().cpu().numpy())
        if y_test!=None:
            y_test = self.scaler.inverse_transform(y_test.detach().cpu().numpy())
