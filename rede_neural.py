...
for i in range(len(y_train_scaled)-lookback):
    ...
    y_train_list.append(y_train_scaled[i+lookback])

#Separar em sequências para o LSTM - Treino
X_test_list = []
y_test_list = []
lookback = 60
for i in range(len(y_test_scaled)-lookback):
    X_test_list.append(y_test_scaled[i:(i+lookback)])
    y_test_list.append(y_test_scaled[i+lookback])
...
y_train_real = scaler.inverse_transform(y_train)  # ✅ Adicione isso
...
df_train_full = pd.DataFrame({
    'Date': train_dates,
    'Price': y_train_real.flatten(),  # ✅ Valores reais em R$
    'Type': 'Treino'
})

df_test_full = pd.DataFrame({
    'Date': test_dates,
    'Price': y_test_real.flatten(),  # ✅ Valores reais em R$
    'Type': 'Real (Teste)'
})

df_pred_full = pd.DataFrame({
    'Date': test_dates,
    'Price': y_pred_real.flatten(),  # ✅ Valores reais em R$
    'Type': 'Previsto'
})
...
# Preços reais da lacuna (dados originais)
...
df_gap = pd.DataFrame({
    'Date': gap_dates,
    'Price': gap_prices,
    'Type': 'Real (Teste)'  # Mesma cor que o teste real
})

# Concatenar com o anterior
